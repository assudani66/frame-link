...
const inputVariants = cva("inline-flex space-x-2 items-center justify-center rounded-md text-sm font-medium transition-colors p-2 w-fit",
  {
    variants: {
      variant: {
      ...
      padding: {
        default: "h-10 py-2 px-4 slate-500",
...
export const Input = ({ children, variant, className, padding, ...props }: InputProps) => {
  return (
    <input {...props} className={cn(inputVariants({ variant, className, padding }))}>
      {children}
    </input>
  );
};
